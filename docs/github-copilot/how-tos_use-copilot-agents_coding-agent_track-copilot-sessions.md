...
After you give Copilot a task, it works autonomously in the background to complete it. See [About GitHub Copilot coding agent](/en/copilot/concepts/about-copilot-coding-agent).
...
## [Tracking agent sessions from the agents tab](#tracking-agent-sessions-from-the-agents-tab)
...
To open the [agents tab](https://github.com/copilot/agents?ref_product=copilot&ref_type=engagement&ref_style=text), click to open the agents panel in the navigation bar on any page, then click **View all**.
...
You can start new agent sessions from the tab. See [Asking GitHub Copilot to create a pull request](/en/copilot/how-tos/agents/copilot-coding-agent/asking-copilot-to-create-a-pull-request).

## [Tracking agent sessions from the GitHub CLI](#tracking-agent-sessions-from-the-github-cli)
...
3.  Open Raycast, search for "Copilot," find the **View Tasks** command, then press Enter.
4.  Click **Sign in with GitHub**, then complete the authentication flow. Raycast will re-open.
5.  You'll see a list of your tasks. To navigate to the linked pull request, press Enter. To view the session logs, press Command+L.
...
You can also start new agent sessions from Raycast. See [Asking GitHub Copilot to create a pull request](/en/copilot/how-tos/use-copilot-agents/coding-agent/create-a-pr#asking-copilot-to-create-a-pull-request-from-raycast).

## [Tracking sessions from Visual Studio Code](#tracking-sessions-from-visual-studio-code)
...
You can also start new agent sessions from Visual Studio Code. See [Asking GitHub Copilot to create a pull request](/en/copilot/how-tos/agents/copilot-coding-agent/asking-copilot-to-create-a-pull-request#asking-copilot-to-create-a-pull-request-from-copilot-chat-in-visual-studio-code).
...
## [Tracking sessions from JetBrains IDEs](#tracking-sessions-from-jetbrains-ides)
...
You can see all of Copilot's sessions by clicking the **GitHub Coding Agent Jobs** button in the sidebar or by clicking the **Open Job List** button after delegating a task to Copilot from GitHub Copilot Chat.
...
## [Tracking sessions from Eclipse](#tracking-sessions-from-eclipse)
...
## [Tracking sessions from GitHub Mobile](#tracking-sessions-from-github-mobile)
...
2.  Use the buttons at the top of the list to filter the list of pull requests.
    ...
    ![Screenshot of the status dropdown list with a check mark against 'Open'.](/assets/cb-305338/images/help/copilot/coding-agent/mobile-status-dropdown.png)
    

## [Tracing commits to session logs](#tracing-commits-to-session-logs)

Every commit from Copilot coding agent is authored by Copilot, with the human who started the task marked as the co-author. Each commit message includes a link to the session logs for that commit.

This gives you a permanent link from any agent-authored commit back to the full session logs, so you can understand why Copilot made a change during code review or trace it later for auditing purposes.

## [Using the session logs to understand Copilot's approach](#using-the-session-logs-to-understand-copilots-approach)
...
## [Steering a Copilot session from the agents tab](#steering-a-copilot-session-from-the-agents-tab)
...
## [Stopping a Copilot session](#stopping-a-copilot-session)

You can stop Copilot from continuing to work on a task by clicking **Stop session** in the session log viewer.

![Screenshot of the log viewer with the 'Stop session' button highlighted.](/assets/cb-32716/images/help/copilot/coding-agent/log-stop-session.png)
...
-   [Troubleshooting GitHub Copilot coding agent](/en/copilot/using-github-copilot/coding-agent/troubleshooting-copilot-coding-agent)