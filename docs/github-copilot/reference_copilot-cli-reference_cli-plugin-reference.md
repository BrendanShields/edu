...
Install a plugin. See [Plugin specification for `install` command](#plugin-specification-for-install-command) below.
...
### [Plugin specification for `install` command](#plugin-specification-for-install-command)
...
`name` (required), `email` (optional), `url` (optional).
...
Path(s) to agent directories (`.agent.md` files).
...
Path(s) to skill directories (`SKILL.md` files).
...
Path to a hooks config file, or an inline hooks object.
...
Path to an MCP config file (e.g., `.mcp.json`), or inline server definitions.
...
Path to an LSP config file, or inline server definitions.

### [Example `plugin.json` file](#example-pluginjson-file)
...
### [Example `marketplace.json` file](#example-marketplacejson-file)
...
### [`marketplace.json` fields](#marketplacejson-fields)
...
Kebab-case marketplace name. Max 64 chars.
...
`{ name, email? }` — marketplace owner info.
...
List of plugin entries (see the table below).
...
`{ description?, version?, pluginRoot? }`

#### [Plugin entry fields (objects within the `plugins` array)](#plugin-entry-fields-objects-within-the-plugins-array)
...
Kebab-case plugin name. Max 64 chars.
...
Where to fetch the plugin (relative path, GitHub, or URL).
...
Plugin description. Max 1024 chars.
...
Path to hooks config or inline hooks object.
...
Path to MCP config or inline server definitions.
...
Path to LSP config or inline server definitions.
...
When `true` (the default), plugins must conform to the full schema and validation rules. When `false`, relaxed validation is used, allowing more flexibility—especially for direct installs or legacy plugins.
...
`~/.copilot/state/installed-plugins/MARKETPLACE/PLUGIN-NAME` (installed via a marketplace) and `~/.copilot/state/installed-plugins/PLUGIN-NAME` (installed directly)
...
`plugin.json`, `.github/plugin/plugin.json`, or `.claude-plugin/plugin.json`
...
`agents/` (default, overridable in manifest)

Skills

`skills/` (default, overridable in manifest)
