Using GitHub Copilot to reduce technical debt - GitHub Docs
...
-   A Copilot subscription with access to Copilot coding agent
...
-   Leveraging Copilot coding agent for large-scale cleanup tasks
...
## [Understanding the technical debt problem](#understanding-the-technical-debt-problem)
...
## [Using Copilot in your IDE for in-the-moment fixes](#using-copilot-in-your-ide-for-in-the-moment-fixes)
...
### [Quick refactoring workflow](#quick-refactoring-workflow)
...
### [Example: Standardizing error handling](#example-standardizing-error-handling)
...
Copilot prompt[](https://github.com/copilot?prompt=%2F%2F%20Highlight%20this%20code%0Atry%20%7B%0A%20%20await%20fetchData\(\)%3B%0A%7D%20catch%20\(e\)%20%7B%0A%20%20console.log\(e\)%3B%0A%7D%0A%0ARefactor%20this%20to%20use%20structured%20logging%20and%20proper%20error%20handling)
...
This response is an example. Copilot Chat responses are non-deterministic, so you may get a different response if you run the same prompt against the same code.
...
## [Using Copilot coding agent for large-scale refactoring](#using-copilot-coding-agent-for-large-scale-refactoring)

Some refactoring tasks are just too big to complete while everyone on the team is busy developing new features. In this situation you can use Copilot coding agent to handle these tasks autonomously. Human effort will still be required—at a minimum for reviewing the changes Copilot coding agent proposes—but getting Copilot to do the bulk of the work can allow you to carry out large-scale refactoring with much less impact on your team's productivity.

### [When to use Copilot coding agent](#when-to-use-copilot-coding-agent)

Use Copilot coding agent for tasks that:
...
### [Workflow for Copilot coding agent](#workflow-for-copilot-coding-agent)
...
5.  Leave comments if changes are needed—Copilot coding agent will update the pull request based on your feedback.
...
For more information, see [Asking GitHub Copilot to create a pull request](/en/copilot/how-tos/use-copilot-agents/coding-agent/create-a-pr#assigning-an-issue-to-copilot) and [Reviewing a pull request created by GitHub Copilot](/en/copilot/how-tos/use-copilot-agents/coding-agent/review-copilot-prs).

### [Safety guardrails](#safety-guardrails)

Copilot coding agent operates with built-in safety measures:
...
## [Creating custom instructions for your team](#creating-custom-instructions-for-your-team)
...
### [Example custom instructions](#example-custom-instructions)
...
### [Benefits of custom instructions](#benefits-of-custom-instructions)
...
## [Running a pilot program](#running-a-pilot-program)

Start small to validate Copilot's impact on your technical debt before rolling it out widely.

### [Week 1: Set up and establish baselines](#week-1-set-up-and-establish-baselines)

1.  Ensure all pilot participants have Copilot access with Copilot coding agent enabled.
...
### [Weeks 2–4: Run the pilot](#weeks-24-run-the-pilot)
...
4.  Assign larger cleanup tasks to Copilot coding agent.
...
### [Week 5: Evaluate results](#week-5-evaluate-results)
...
-   Which types of code change suggestions, made by Copilot coding agent in pull requests, did developers accept most often?
...
### [Copilot suggests incorrect changes](#copilot-suggests-incorrect-changes)
...
-   Leave detailed feedback in pull request reviews to allow Copilot coding agent to fix the problems

### [Pull requests are too large to review](#pull-requests-are-too-large-to-review)

If Copilot coding agent creates pull requests that are difficult to review:

-   Break large tasks into smaller, focused issues
-   Ask Copilot coding agent to handle one file or directory at a time
-   Use more specific issue descriptions

### [Changes break tests](#changes-break-tests)

If refactoring introduces test failures:

-   Ensure your test suite runs reliably before using Copilot coding agent
-   Review Copilot changes carefully before merging
-   Ask Copilot to update tests along with the code changes

### [Team adoption is slow](#team-adoption-is-slow)
...
-   Assign large refactoring tasks to Copilot coding agent
...
-   **Automate dependency updates**: Create recurring issues for Copilot coding agent to handle dependency updates.
...
-   [GitHub Copilot coding agent](/en/copilot/using-github-copilot/coding-agent)
...
-   [5 ways to integrate GitHub Copilot coding agent into your workflow](https://github.blog/ai-and-ml/github-copilot/5-ways-to-integrate-github-copilot-coding-agent-into-your-workflow/) in the GitHub blog