...
The Agent Client Protocol (ACP) is a protocol that standardizes communication between clients (such as code editors and IDEs) and coding agents (such as Copilot CLI). For more details about this protocol, see the [official introduction](https://agentclientprotocol.com/get-started/introduction).
...
## [Starting the ACP server](#starting-the-acp-server)

GitHub Copilot CLI can be started as an ACP server using the `--acp` flag. The server supports two modes, `stdio` and `TCP`.

### [stdio mode (recommended for IDE integration)](#stdio-mode-recommended-for-ide-integration)
...
## [Integrating with the ACP server](#integrating-with-the-acp-server)
