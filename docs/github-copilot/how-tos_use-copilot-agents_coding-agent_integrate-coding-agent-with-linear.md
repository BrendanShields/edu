...
The Copilot integration in Linear allows you to invoke Copilot coding agent without leaving your Linear workspace. From within a Linear issue you can initiate coding agent sessions and open pull requests, using the context of your issue description and comments.
...
## [Installing the Copilot app in Linear](#installing-the-copilot-app-in-linear)

Note

In order to install the Copilot app in Linear, be an owner of the organization or enterprise where you want to install the app. You must also be a workspace admin in Linear.
...
## [Using the Copilot app in Linear](#using-the-copilot-app-in-linear)

The first time you use the Copilot app in Linear, you will need to connect it to your GitHub account. You will also need to specify a repository for Copilot coding agent to use. Only users with **write** access to the specified repository can trigger Copilot coding agent to work in that repository. Contributors to the issue without repository **write** access can help guide Copilot by providing input to the issue conversation, which will be used as context when creating the pull request.

1.  In Linear, create an issue where you want to use Copilot coding agent.
2.  Click the **Assign** dropdown, then select **GitHub Copilot**.
3.  If you haven't yet specified a repository for Copilot coding agent to use, you will be prompted to do so now. This is where Copilot coding agent will open the pull request related to this issue.
...
5.  In the "Links" section of your Linear issue, you will now see a linked "\[WIP\]" pull request created by Copilot coding agent. Click the link to view the pull request on GitHub.
6.  Once Copilot coding agent has finished working on the pull request, a notification will be added to the "Activity" section of your Linear issue.
