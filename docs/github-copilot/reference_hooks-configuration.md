...
### [User prompt submitted hook](#user-prompt-submitted-hook)
...
# Log the tool use
echo "$(date): Tool=$TOOL\_NAME Args=$TOOL\_ARGS" >> tool-usage.log

# Check for dangerous patterns
if echo "$TOOL\_ARGS" | grep -qE "rm -rf /|format|DROP TABLE"; then
...
if \[ "$TOOL\_NAME" = "edit" \]; then
  PATH\_ARG=$(echo "$INPUT" | jq -r '.toolArgs' | jq -r '.path')

  if \[\[ ! "$PATH\_ARG" =~ ^(src/|test/) \]\]; then
...
INPUT=$(cat)
TOOL\_NAME=$(echo "$INPUT" | jq -r '.toolName')
RESULT\_TYPE=$(echo "$INPUT" | jq -r '.toolResult.resultType')

# Track statistics
echo "$(date),${TOOL\_NAME},${RESULT\_TYPE}" >> tool-stats.csv

# Alert on failures
if \[ "$RESULT\_TYPE" = "failure" \]; then
  RESULT\_TEXT=$(echo "$INPUT" | jq -r '.toolResult.textResultForLlm')
  echo "FAILURE: $TOOL\_NAME - $RESULT\_TEXT" | mail -s "Agent Tool Failed" admin@example.com
fi
...
### [Error occurred hook](#error-occurred-hook)
...
#!/bin/bash
INPUT=$(cat)
ERROR\_MSG=$(echo "$INPUT" | jq -r '.error.message')
ERROR\_NAME=$(echo "$INPUT" | jq -r '.error.name')

echo "$(date): \[$ERROR\_NAME\] $ERROR\_MSG" >> errors.log
...
### [Multiple hooks of the same type](#multiple-hooks-of-the-same-type)
...
### [Conditional logic in scripts](#conditional-logic-in-scripts)
...
WEBHOOK\_URL="https://hooks.slack.com/services/YOUR/WEBHOOK/URL"

curl -X POST "$WEBHOOK\_URL" \\
  -H 'Content-Type: application/json' \\
  -d "{\\"text\\":\\"Agent Error: $ERROR\_MSG\\"}"
...
echo "$TIMESTAMP,$USER,$TOOL\_NAME" >> /var/log/copilot/usage.csv
...
if \[ "$TOOL\_NAME" = "edit" \] || \[ "$TOOL\_NAME" = "create" \]; then
