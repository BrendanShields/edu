Tracking license activation and initial usage with Copilot usage metrics - GitHub Docs
...
## [Step 1: Access the usage metrics dashboard](#step-1-access-the-usage-metrics-dashboard)
...
## [Step 2: Track license activation](#step-2-track-license-activation)
...
How many licensed developers have used Copilot at least once this calendar month.

Compare to your total assigned licenses. If the number is much lower, verify IDE configuration or communicate activation steps to your teams.

IDE daily active users

How many unique users are using Copilot each day.

Look for an upward trend in the first two weeks after rollout. A flat line may signal that users need additional enablement or setup guidance.

IDE weekly active users

Rolling 7-day total of active users.

Use this to track consistency. Steady or increasing WAU indicates successful activation and recurring use.
...
## [Step 3: Identify early adoption signals](#step-3-identify-early-adoption-signals)
...
Consistent DAU growth

“IDE daily active users” graph

Steady increase in daily users over the first month.

Feature variety

“Requests per chat mode” graph

Developers trying multiple chat modes (Ask, Edit, Plan, Agent) suggests curiosity and engagement.

Initial agent usage

“Agent adoption” card

Even small agent adoption (5–10%) early on is a positive signal of experimentation.

Healthy early adoption usually looks like 60–80% of assigned users showing activity within the first month.

## [Step 4: Act on limited adoption signals](#step-4-act-on-limited-adoption-signals)
...
Low total active users

Users haven’t activated licenses or configured Copilot in their IDE.

Re-share onboarding materials or run a short “activation check” session.

Flat daily usage

Developers tried Copilot once but aren’t returning.

Provide enablement resources like Copilot prompts or internal success stories.

No agent usage

Teams may not know about the Copilot agent.

Share examples of advanced use cases to encourage exploration.

You can also use Copilot Chat to help diagnose issues. For example:

Copilot prompt[](https://github.com/copilot?prompt=What%20patterns%20might%20explain%20low%20adoption%20across%20some%20teams%20in%20the%20Copilot%20metrics%20export%3F)
...
## [Step 5: Track activation programmatically](#step-5-track-activation-programmatically)
...
## [Step 6: Export user-level data for deeper analysis](#step-6-export-user-level-data-for-deeper-analysis)
...
2.  Download the NDJSON file and use [Copilot Chat](https://github.com/copilot?ref_product=copilot&ref_type=engagement&ref_style=text) to analyze the data. For example, ask:
    ...
    Copilot prompt[](https://github.com/copilot?prompt=*%20Summarize%20which%20organizations%20show%20the%20largest%20increase%20in%20%60loc_added_sum%60%20this%20month.%0A%20*%20Identify%20users%20with%20high%20%60user_initiated_interaction_count%60%20but%20low%20%60code_acceptance_activity_count%60.)
    
     \* Summarize which organizations show the largest increase in \`loc\_added\_sum\` this month.
     \* Identify users with high \`user\_initiated\_interaction\_count\` but low \`code\_acceptance\_activity\_count\`.
