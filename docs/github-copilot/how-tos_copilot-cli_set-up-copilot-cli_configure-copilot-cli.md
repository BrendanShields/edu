...
## [Setting trusted directories](#setting-trusted-directories)
...
### [Choosing to trust a directory](#choosing-to-trust-a-directory)
...
### [Editing trusted directories](#editing-trusted-directories)
...
## [Setting allowed tools](#setting-allowed-tools)
...
### [Allowing a tool for the first time](#allowing-a-tool-for-the-first-time)
...
#### [Allowing all tools](#allowing-all-tools)
...
#### [Allowing a tool](#allowing-a-tool)
...
### [Specifying which tool you want to allow or deny](#specifying-which-tool-you-want-to-allow-or-deny)
...
#### [Allowing or denying shell commands](#allowing-or-denying-shell-commands)
...
#### [Allowing or denying `'write'` tools](#allowing-or-denying-write-tools)
...
#### [Allowing or denying MCP server tools](#allowing-or-denying-mcp-server-tools)
...
### [Allowing some tools while denying others](#allowing-some-tools-while-denying-others)
...
### [Limiting available tools](#limiting-available-tools)
...
## [Setting path permissions](#setting-path-permissions)
...
### [Allowing access to all paths](#allowing-access-to-all-paths)

To disable path verification and allow access to any path, use the `--allow-all-paths` flag when starting Copilot CLI.

### [Disallowing access to the temp directory](#disallowing-access-to-the-temp-directory)

To disallow access to the temp directory, use `--disallow-temp-dir`.

## [Setting URL permissions](#setting-url-permissions)
...
### [Disabling URL verification](#disabling-url-verification)

To disable URL verification, use the `--allow-all-urls` flag.

### [Pre-approving specific domains](#pre-approving-specific-domains)

To pre-approve specific domains, use `--allow-url=DOMAIN`.

-   For example, `--allow-url=github.com`.

### [Denying specific domains](#denying-specific-domains)

To deny specific domains, use `--deny-url=DOMAIN`.

-   For example, `--deny-url=github.com`.

## [Allowing all tools, paths, and URLs](#allowing-all-tools-paths-and-urls)
