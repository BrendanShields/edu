...
-   **Accessing remote-only toolsets** such as `copilot` (for Copilot coding agent) and `github_support_docs_search`, which are only available on the remote MCP server
...
## [Configuring toolsets for the remote MCP server](#configuring-toolsets-for-the-remote-mcp-server)
...
## [Configuring toolsets for the local MCP server](#configuring-toolsets-for-the-local-mcp-server)
...
-   [Using the GitHub MCP Server](/en/copilot/how-tos/provide-context/use-mcp/use-the-github-mcp-server)